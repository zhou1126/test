...
def search_query_gen(client_def, prompt, selected_model, temperature, top_p):
    search_for = client_def.chat.completions.create(
                        ...
                        messages=[
                            {"role": "system", "content": '''You are a Google engineer and will receive a prompt with a list of company names. Find the corresponding search query on Google per company. Do not show me the corresponding webpage. Show me the search query in bullet points and separated by "\n".'''},
...
# def search_web_gen(client_def, prompt, selected_model, temperature, top_p):
#     search_for = client_def.chat.completions.create(
#                         model=selected_model,
#                         messages=[
#                             {"role": "system", "content": '''You are a Google engineer and will receive a prompt with a list of company names. Find the corresponding corporate news or update webpages per company. Show me the webpage in bullet points and separated by "\n".'''},
#                             {"role": 'user', "content": prompt}]
#                         ,
#                         temperature=temperature,
#                         top_p=top_p,
...
# user_input = 'Here is a list of companies: Brother, Panduit. Search 2024 News and updates for the United States from the their companies web.'
# search_web_list = search_web_gen(client, user_input, selected_model, 0, 0.95)
# print(search_web_list)